...
class AdaptiveWriterAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            calendar_suggestions = state.response_metadata.get("calendar_suggestions", [])
            if calendar_suggestions:
                context_parts.append("\n📅 Calendar Suggestions Found:")
                for i, slot in enumerate(calendar_suggestions[:2], 1):
                    context_parts.append(f"  - Option {i}: {slot.get('date')} at {slot.get('time')} ({slot.get('duration')} minutes)")
            elif state.calendar_data and state.calendar_data.meeting_request:
